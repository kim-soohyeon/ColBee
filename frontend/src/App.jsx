...
function App() {
  ...
  const getRefrigeratorStatus = () => {
    if (ingredients.length === 0) return 'inactive';
  ...
  const statusCounts = {
    ...
    inactive: ingredients.length === 0 ? 1 : 0
  };
    ...
    <div className="container">
      ...
      <header className="app-header">
        <div className="header-content">
          ...
          <p className="app-motto">Never Waste Again with ColBee</p>
          <p className="app-subtitle">"버리지 않는 냉장고, ColBee와 함께 하세요!"</p>
        </div>
      </header>
...
function HomeView({ statusCounts, onNavigate }) {
  return (
    <div className="view-container">
      ...
      {/* 월간 보고서 요약 추가 (Mascot와 겹치지 않도록 하단에 배치) */}
      <div className="card">
        <h2 className="card-title">📈 이달의 리포트 요약</h2>
        <div className="stats-header" style={{ marginBottom: 0, padding: '1rem' }}>
          <div className="stats-emoji" style={{ fontSize: '32px' }}>😊</div>
          <div className="stats-message">
            <h3 style={{ fontSize: '16px' }}>"정말 잘하고 있어요!"</h3>
            <p style={{ fontSize: '12px' }}>쓰레기율 16% (저번 달 대비 -2%)</p>
          </div>
        </div>
        <button
